...
func TestPlanStoreBindsOwnerAndPreventsConcurrentClaims(t *testing.T) {
	store := NewPlanStore()
	if err := store.putScoped("plan", "actor/session", "target", "payload"); err != nil {
		t.Fatal(err)
	}
	if _, ok := store.claim("plan", "other/session"); ok {
		t.Fatal("different owner claimed plan")
	}
	if _, ok := store.claim("plan", "actor/session"); !ok {
...
func TestPlanStorePurgesExpiredEntriesBeforeCapacityCheck(t *testing.T) {
	...
	if err := store.putScoped("fresh", "", "", "payload"); err != nil {
		t.Fatal(err)
	}
