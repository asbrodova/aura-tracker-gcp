...
By default all 62 tools are registered. Use the `--modules` flag to load only the services you work with — each excluded module also skips its GCP client connection at startup.
...
- **MCP annotations**: all 62 tools carry standard `readOnlyHint` / `destructiveHint` / `idempotentHint` annotations — clients like Claude Desktop use these to decide whether to present a confirmation UI before calling a tool
...
│       ├── tools/                 # one file per GCP domain (62 tools)
